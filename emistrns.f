...
c
c--------------Added by Kristina 08/04/08-----------------------------
c
c      pointer (p,modvdep)
      real savdep(MXCOL1,MXROW1,MXTRK*MXSOUR)
c      dimension modvdep(ncol(igrd),nrow(igrd),nspec)
      integer saspc,modspc,i,j,n,s,loc
      integer nx, ny, nz, ispc
c      real saconc(MXCOL1,MXROW1,MXLAY1,MXTRK*MXSOUR)
c
c---------------End added 08/04/08------------------------------------
c
...
c
      if (lApp) then
c
c     ASSIGN vdep array for apportionment species
c
...
c
        ...
        call Appalign(conc(iptr4d(igrd)))
