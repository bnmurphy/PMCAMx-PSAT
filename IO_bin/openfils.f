...
c--------not binary----------
c        notbin = 'nb'       
c        open(unit=30,file=filroot(1:ii+5)//notbin,form='FORMATTED',
...
c
c-------Open and create the Apportionment output files
c
      if(lApp) then
c
c       INSTANTANEOUS output file
c
        write(filroot(ii+1:),'(A)') '.App'
        filtmp = filroot
        iAppo = nfils
        nopen = nopen + 1
        action = 'Opening output APPORTIONMENT file for coarse grid.'
        open(unit=iAppo,file=filroot(1:ii+5),form='UNFORMATTED',
     &                                       status= 'UNKNOWN',ERR=7005)
        write(iout,9000)'Output APPORTIONMENT file coarse grid (unit):',
     &                                                          iAppo
        write(iout,9002) '   File: ',filroot(1:ii+5)
        nfils = nfils + 1
c        
...
c
        write(filroot(ii+1:),'(A)') '.AppAve'
        filtmp = filroot
        iAppoA = nfils
        nopen = nopen + 1
        action = 'Opening output APPORTIONMENT file #2 for coarse grid.'
        open(unit=iAppoA,file=filroot(1:ii+5),form='UNFORMATTED',
     &                                       status= 'UNKNOWN',ERR=7005)
...
c
     ...
     &                                                         nfils
        ...
        nfils = nfils + 1
c
c--------------Added by Kristina 04/04/07------------------------
c
        action = 'Reading restart app. filename for coarse grid.'
        irec = irec + 1
        read(inp,'(20X,A)',ERR=7001,END=7004) filtmp 
        iAppinst = nfils
        inquire(file=filtmp,exist=lexist)
        if( .NOT. lexist .OR. filtmp .EQ. ' ' ) goto 7002
        nopen = nopen + 1
        open(unit=iAppinst,file=filtmp,form='UNFORMATTED',
     &                                    status='OLD',ERR=7005)
        write(iout,9000)'Coarse grid app restart file         (unit):',
     &                                                         nfils
        write(iout,9002) '   File: ',filtmp(:istrln(filtmp))
        nfils = nfils + 1
c
c--------------End Added 04/04/07--------------------------------
c
        if( nnest .GT. 0 ) then
